...
export function loadLayoutWithCharts(layoutName) {
    console.log('Loading layout with charts for layout name:', layoutName);
    window.loading = true;
    const savedLayouts = JSON.parse(localStorage.getItem('savedLayouts'));
    ...
    if (layoutData) {
        ...
        setTimeout(() => {
            ...
            console.log('Layout loaded and Highcharts initialized.');
        }, 1000);
    } else {
        alert("Layout not found.");
        console.error('No layout data found for the given layout name.');
        window.loading = false;
    }
}
